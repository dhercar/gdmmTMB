...
For this example, we use two datasets available as part of the `gdm` package. We start with two matrices: 
...
By specifying a binomial distribution, the Jaccard dissimilarity index is modelled as the probability $\pi$ that a randomly chosen species from the combined species pool ($a_{ij} + b_i + b_j$) is not shared between observations $i$ and $j$. 
