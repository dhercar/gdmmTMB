...
We start with two matrices: A community matrix (site x species) and a
...
In this example, we specify a binomial distribution and estimate model
parameters with a Bayesian bootstrapping approach. Monotonic I-splines
for dissimilarity gradients (`diss_formula`) are specified via `isp(x)`
and `mono = TRUE`.
...
```
    ...
    ## call:
    ## 
    ##  gdmm(Y = sp[, -1], X = env, diss_formula = ~isp(bio6) + isp(bio15) +     isp(bio19), mono = TRUE, family = "binomial", bboot = TRUE,     n_boot = 100)
    ## 
    ## --------------------------------- Coeff. Table --------------------------------- 
    ## 
